...
%April 2, 2021: 
%this code is a modified version used for creating the stats files for wood
%(N_I_wood, and delta_wood)
...
switch worker
    ...
    case 4
        start_pos=94;
        %end_pos=124;
        end_pos=98; %for 7 x 7 grid at Bartlett
    case 5
        start_pos=125;
        end_pos=155;
    case 6
...
for pos=start_pos:end_pos
    ...
    file_rxp=strcat(path,'ScanPos',num2str(pos),'lewos.bin');
    file_rxp_res=strcat(path_res,'ScanPos',num2str(pos),'lewos.bin');
...
%     fidres_gap=fopen(file_res_gap,'w');
    file_res_hit=strcat(file_rxp_res,'_statistics_wood_hits.txt');
    ...
    Ni_wood=zeros(dimX*dimY*dimZ,1);
    ...
    while ~feof(fid_rxp)
        ...
        event=fread(fid_rxp,1,'int'); %April 2021: I was wondering why this was not causing a problem here, I rememeber now that when there is a hit the sequence is:
        % 0 (pulse emitted) xyz orig, xyz dir
        % then there is a new line with: 1 xyz position of hit
        %since I only recorded the first hits, the next line is
        %necessarely: 0 xyz orig xyz dir
        %     event_a=fscanf(fid_rxp_ascii, '%d',[1]);
...
%             lineCoords = [OrigX OrigY OrigZ intersection_bbox]; %line is set by pulse origin and intersection with bbox
        ...
        if event==2 %hit is wood
            ...
            Ni_wood(indexes(size(indexes,1)))=Ni_wood(indexes(size(indexes,1)))+1;
            %count=count+1;
...
%     end
    ...
    for i=1:size(index,1)
        ...
        fprintf(fidres_hit,'%.2f %.2f %.2f %.2f %.2f %.2f %.3f %d\n',vx(1),vy(1),vz(1),vx(2),vy(2),vz(2),pathlenghts_hits(index(i)),Ni_wood(index(i)));
