...
max_lad=99; %arbitrary for wood

voxel_size=0.3;

max_tree_height=120; %this is only for setting a max size on a vector

occlusion_threshold=15; %for wood it is not relevant to correct this, but there is a risk to overestimate if probe lenght is low, reducing from 30 to 15
...
file_will_be_resampled=0; %using 0 does not include lad=0 points, using 1 does include them. Nov 2019: why would I want to include them is not clear

plot_size=48;%60; %this is used only for the cluster, irrelevant for individual trees
...
%         end
        ...
        %voxel
        %file_LAD=strcat(path,plot,'_',grid,'mgrid_LAD_no_zeros.asc')  %for Mac
        file=strcat(path,plot,'_',grid,'mgrid_WAD_no_zeros.asc')  %for Mac
        fid=fopen(file);
        file_pathlenght=strcat(path,plot,'_',grid,'mgrid_pathlenght_wood.asc')  %for Mac
...
%             file_ground=strcat(file_LAD,'_ground_level_.asc');
%             fid_ground=fopen(file_ground);
...
%             end
            ...
            %April 2021: this loop creates a LAD profile
            while ~feof(fid)
                xmin=fscanf(fid, '%f', [1]);
                if isempty(xmin), break, end
                ymin=fscanf(fid, '%f', [1]);
...
%                 end
                ...
                switch file_will_be_resampled
                    ...
                    case 0
                        if lad>0, fprintf(fidres, '%.3f %.3f %.3f %.3f %.3f %.3f %f\n',xmin,ymin,zmin,xmax,ymax,zmax,lad);end
                    case 1
                        fprintf(fidres, '%.3f %.3f %.3f %.3f %.3f %.3f %f\n',xmin,ymin,zmin,xmax,ymax,zmax,lad);
                end
...
%             %fidres_omega=fopen(fileres_omega,'w');
%             
%             sum_la_=zeros(max_tree_height*10,1);
%             sum_la_omega=zeros(max_tree_height*10,1);
%             sum_lad_=zeros(max_tree_height*10,1);
...
%             
             ...
             LAI_profile=sum_la_./plot_size^2;
...
%             fprintf(fidres_stats, 'Total leaf area after considering within voxel clumping: %.0f m2\nLAI considering within voxel clumping : %.3f m2/m2\n',total_leaf_area_omega_v, LAI_occ_corrected_omega);
            ...
            fprintf(fidres_stats, 'height (m)/leaf area per layer/leaf area index per layer/number of occluded voxels/number of voxels containing material/proportion of occluded voxels per layer\n');
            for h=1:max_tree_height*10
                if sum_la_(h)>0,fprintf(fidres_stats, '%.2f %.1f %.3f %d %d %.4f \n',(h-1)/10+voxel_size/2,sum_la_(h),LAI_profile(h),occluded(h),count_(h),occluded(h)/count_(h));end
            end
