...
unsigned int *intersectedVoxelData; //storage for the indexes of the intersected voxels - dynamically allocated
int addedVoxelCount;

double xStart, yStart, zStart, xEnd, yEnd, zEnd; // point where segment intersects the grid ***MB: this is what I need? what grid? is this the outer limist of the array? if it is this could be the way to consider only the line starting point, and ending point if there is a hit
...
plhs[0] = mxCreateNumericMatrix(storageSize,1,mxUINT32_CLASS,mxREAL); // initializes to zeros
intersectedVoxelData = (unsigned int*)mxGetData(plhs[0]);
...
while ((X!=Xend)||(Y!=Yend)||(Z!=Zend))
{
    ...
    if (addedVoxelCount>storageSize)
    {
        ...
        intersectedVoxelData = (unsigned int*)mxRealloc(intersectedVoxelData, sizeof(unsigned int)*storageSize);
    }
    intersectedVoxelData[addedVoxelCount-1] = sub2ind(X, Y, Z, gridSize);
}
// Update the size of the output matrix
intersectedVoxelData = (unsigned int*)mxRealloc(intersectedVoxelData, sizeof(unsigned int)*addedVoxelCount);
...
{
	return (Y + (X - 1)*gridSize[1] + (Z - 1)*gridSize[1]*gridSize[0]);
}
//MB: I changed the 0 to a 1 for first gridsize index, as per bug repported on the file exchage comments. pour les matrices carrés le probleme ne sera pas observable, car la dim x et y sont egales. Turned back since the suggested correction was not triggering an error and results were wrong on non square matrix floor
