...
grid=5 %this is the distance between scasn positions, used to simulate different field protocols. Values 5 , 10 , 15 or 20. Use 5 to consider all scan positions
occlusion_threshold=30; %this parameter refers to Beland and Kobayashi 2021
%plot_size=60;

path_rpy=strcat('F:\ReadRXP\voxTrace_',plot,'\');
%path_stats=strcat('F:\ReadRXP\voxTrace_',plot,'\');
path_stats=path_rpy; %strcat('E:\',plot,'\');
...
if strcmp(plot,'MorganMonroe')
    ...
    height(2)=9999;
    g_height(2,:)=uniform;
    FV_code(2)=1; %uniform not available in Fliesvox!!
...
file=strcat(path_stats,plot,grid_size,'_LAD_no_zeros.asc');
...
file=strcat(path_stats,plot,grid_size,'_pathlenght.asc');
...
file=strcat(path_stats,plot,grid_size,'_statistics.asc');
...
file=strcat(path_stats,plot,grid_size,'_LAD_stabilization.csv');
...
for pos=scan_pos(1,:)  %begin loop through scan position
    ...
    file_stats_hits=strcat(path_stats,'ScanPos',num2str(pos),'lewos.bin_statistics_hits.txt');
    fid_stats_gaps=fopen(file_stats_gaps);
...
for idx=(voxels_containing_material).'  %this last .' is to convert the vector from a column to a line, so that the for loop considers each element of idx seperately instead of passing idx as one vector
    ...
    fprintf(fidres_lad, '%.3f %.3f %.3f %.3f %.3f %.3f %f %d\n',vx(1),vy(1),vz(1),vx(2),vy(2),vz(2),Master(idx,4),g_code(idx));
